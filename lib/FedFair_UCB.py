...
class FedFair_UCB:
    def __init__(self, dimension, alpha, lambda_, NoiseScale, delta_1=1e-2, delta_2=1e-1, neighbor_identification_alpha=1e-2,
                 dataSharing = False, aggregationMethod="combine"):
    ...
    def testConnectednessBetweenTwoUsers(self, currentUser, neighborUser):
        ...
        Test whether two user models have sufficiently similar ground-truth thetas
        :param currentUser:
        :param neighborUser:
        :return:
        """
...
# time dependent oracle vs data dependent oracle
# algorithm requires comping pairwise edges which is n^2. 
# still haven't done the communication (are we doing event triggered synchronous)


# data-independent vs data-dependent
# 1. Compute epsilon = 1/sqrt(t)
# 2. Build the connections graph using \epsilon and \theta*
# 3. Find the clique of this graph that maximizes
    # b. Data-dependent: the maximum of determinant ratio (determinant is expensive to compute as well?) (NP Complete but no approximations exist)


# what have I done so far for the simulation

# coded up the pairwise test and can construct the user graph
# however, for the rejection sampling I've designated a specified number of clusters and each cluster is comprised of clients within epsilon based on Capital T
# was thinking we vary epsilon with T and sample that way, but that's actually bad, because given the computational complexity of our algorithm, we cannot practically have too many clients
# don't experience this jumping around behavior for the data-independent oracle
# run disLinUCB on the distributed data
